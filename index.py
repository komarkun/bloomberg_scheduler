...
request_payload = {
    ...
    'universe': {
        '@type': 'Universe',
        'contains': [
            {'@type': 'Identifier', 'identifierType': 'TICKER', 'identifierValue': 'LQ45 INDEX'},
            {'@type': 'Identifier', 'identifierType': 'TICKER', 'identifierValue': 'JCI INDEX'},
        ]
    },
    'fieldList': {
    ...
    'runtimeOptions': {
        ...
        'dateRange': {
            ...
            'startDate': (datetime.datetime.now() - datetime.timedelta(days=4)).strftime('%Y-%m-%d'),
...
new_df = pd.DataFrame({
    'index_code': df['IDENTIFIER'].str.replace(' INDEX', ''),  # Remove ' INDEX' suffix
    'report_date': df['DATE'],
...
try:
    result = supabase.table('bloomberg_stock_index_histories').upsert(records).execute()
